...
function apiBaseUrl() {
  return window.$api?.getApiBaseUrl?.() || '/api';
}
  ...
  (error) => {
    if (error.response?.status === 409 && error.response?.data?.code === 'DATA_SOURCE_CHANGED') {
      ...
      if (location.hash !== '#/login') {
        location.href = routeUrl(`/login?oauthError=${encodeURIComponent(message)}`);
      }
    }
    if (error.response?.status === 401) {
      ...
      if (location.hash !== '#/login') {
        const message = error.response.data?.code === 'SESSION_REPLACED'
          ...
          : '';
        location.href = message ? routeUrl(`/login?oauthError=${encodeURIComponent(message)}`) : routeUrl('/login');
      }
    }
...
export function oauthStartUrl(provider: PlatformKey, dataSourceId?: number) {
  ...
  return `${apiBaseUrl()}/auth/oauth/${provider}/start${query}`;
}
