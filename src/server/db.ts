...
function resolveSqlJsFile(file: string) {
  ...
  const candidates = [
    process.env.SQLJS_WASM_PATH ? path.join(process.env.SQLJS_WASM_PATH, file) : '',
    resourcesPath ? path.join(resourcesPath, file) : '',
    resourcesPath ? path.join(resourcesPath, 'sqljs', file) : '',
...
function loadSqlJs() {
  ...
  const resourcesPath = (process as NodeJS.Process & { resourcesPath?: string }).resourcesPath;
  const candidates = [
    resourcesPath ? path.join(resourcesPath, 'sql-wasm.js') : '',
  ...
  for (const candidate of candidates) {
    try {
      const loaded = runtimeRequire(candidate);
      return (loaded.default || loaded) as (config?: SqlJsConfig) => Promise<SqlJsStatic>;
    } catch (error) {
      if (candidate === 'sql.js') throw error;
    }
  }

  throw new Error('无法加载 SQL.js 运行文件');
}
...
export async function initDatabase() {
  ...
  const SQL = await initSqlJs({ locateFile: resolveSqlJsFile });
