...
function hasOAuthConfig(item: { platform: string; config: Record<string, string> }) {
  if (item.config.loginEnabled === 'false') return false;
  if (item.platform === 'dingtalk') return Boolean(item.config.appKey && item.config.appSecret);
  if (item.platform === 'feishu') return Boolean(item.config.appId && item.config.appSecret);
  return false;
}
...
router.get('/auth/oauth/:provider/start', async (req, res, next) => {
  ...
  try {
    ...
    runtimeSettings = await getRuntimeSettings(requestBaseUrl(req));
    ...
    if (runtimeSettings) {
      const message = error instanceof Error ? error.message : '企业登录启动失败';
      res.redirect(frontendLoginErrorUrl(message, runtimeSettings));
      return;
    }
    next(error);
  }
